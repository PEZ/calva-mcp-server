...
(deftest-async command-registration
  (testing "MCP server commands are registered"
    (try
      (p/let [#_#__ (vscode/commands.executeCommand "calva-backseat-driver.startServer")
              pre-activation (vscode/commands.getCommands true)]
        (is (= false
               (.includes pre-activation "calva-backseat-driver.startServer"))
            "there is no start server command before activation")
        (is (= false
               (.includes pre-activation "calva-backseat-driver.stopServer"))
        ...
        (p/let [extension (vscode/extensions.getExtension "betterthantomorrow.calva-backseat-driver")
          ...
          (is (= true
                 (.includes post-activation "calva-backseat-driver.startServer"))
              "there is a start server command after activation")
          (is (= true
                 (.includes post-activation "calva-backseat-driver.stopServer"))
...
(deftest-async server-lifecycle
  (testing "MCP server can be started and stopped via commands"
    (-> (p/let [_ (p/delay 500) ; Allow log file to have been created
                ...
                server-info+ (vscode/commands.executeCommand "calva-backseat-driver.startServer")
                ...
                success?+ (vscode/commands.executeCommand "calva-backseat-driver.stopServer")]
          (is (not= nil
                    instance)
